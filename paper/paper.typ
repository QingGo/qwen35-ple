...
#show: icml2024.with(
  ...
  abstract: [
    ...
    Two measurements then point in different directions. A probe of the frozen table recovers about 59% of the trigram top-1 that explicit counts reach on the same corpus, and additional training makes it worse. Measured directly, the vector the reader injects has an effective dimensionality of $1.001$ over 600 prompts, against $1.480$ for the hidden state it is gated on. Opening the reader's arithmetic separates the two reasons that can happen. At the answer position every prompt in a task addresses a byte-identical row, because each task's template ends in a fixed trigram, so the bound is tight there and the null is forced by its premise; twelve tokens back the same read-out is fed an item-specific row of effective dimensionality $136.7$ and emits $1.02$. Both losses are real, and only the second is fixable by improving the reader --- but repairing the read-out does not make table content useful at the position the arms read.
  ],
  ...
  appendix: [
    ...
    *Open-ended degradation.* Without the token-level policy, unconditional PLE fusion on natural-language code prompts produces repetitive fragments, unrelated repository text, and broken structure. The learned policy suppresses fusion on tokens where the n-gram prior is not clearly beneficial, which substantially reduces this degradation.
    ...
    = The Read-Out Collapse, Localised <sec-collapse-app>

    §4.7 reports that the injected vector $c_t$ has an effective dimensionality of
    $1.001$ while the hidden state it is gated on sits at $1.480$, and that the
    argument for calling this a read-out defect rested on a random map of $h_t$.
    That control bounds the measurement, not the explanation: the read-out's input
    is $(h_t, e_t)$, and a faithful read-out fed a constant $e_t$ would emit a
    constant too. Measuring the ladder at several positions back from the answer
    position varies $e_t$ and so separates the two.

    Each stage is the reader's own arithmetic, captured by forward hooks on
    `value_proj` and `out_proj`; the effective dimensionality is the participation
    ratio over the 600 evaluation items, and $cos("real", "shuf")$ is the per-item
    cosine between the real and row-permuted conditions using the same permutation
    as the control arms.
    ...
    #figure(
      ...
      caption: [
        The read-out is close to a constant function of its input, which the
        ladder below localises. #emph[(a)] Effective dimensionality of the
        injected vector, of the hidden state it is gated on, and of that hidden
        state after a random linear map of the same shape. Participation ratio
        $(sum lambda)^2 \/ sum lambda^2$ over $n = 600$ items; error bars on the
        random-map bar are one standard deviation across three maps, and bootstrap
        95% intervals are $[1.0012, 1.0015]$ for $c_t$ and $[1.433, 1.535]$ for
        $h_t$. #emph[(b)] Share of variance in the leading direction.
        #emph[(c)] What actually moves the injected vector.
      ],
    )
    ...
    #figure(
          ...
          caption: [
            The collapse is inherited from the addressing at the answer position
            and produced by the read-out everywhere else. At offset 0 every one of
            the 600 items addresses a byte-identical row, because all three prompt
            templates end in a fixed trigram, so a perfect read-out would still
            emit a constant. At offset 12 the addressed row is item-specific
            ($190$ of $200$ BoolQ rows distinct, PR $136.7$) and the same read-out
            still emits PR $1.02$. Offset 40 is meaningful for BoolQ only: the NQ
            and TriviaQA prompts are shorter than $40$ tokens, so those rows are
            clamped to position 0.
          ],
        )

    Per task at the two informative offsets, addressed rows distinct out of 200
    and the resulting $c_t$:
    ...
    #figure(
          ...
          caption: [
            Where the addressing is degenerate, so is the injection; where the
            addressing is rich, the injection is still degenerate.
          ],
        )
...
#emph[Qualification 4 (tightness).] The bound is vacuous when $I(Y ; w_t | h_t)$ is large, and how large it is depends on where in the prompt the window sits. On content-bearing windows it is not zero: an explicit count trigram over such a window reaches $0.2535$ top-1 against a majority floor of $0.0537$ (§4.6). At the answer position, where the arms actually read and where every prompt in a task ends in that task's template trigram, the addressed row is byte-identical across items, so the right-hand side is zero there and the bound is tight rather than vacuous. A zero content result at that position is therefore forced by the premise; it is not by itself evidence that a read-out could not have transmitted something, and §4.7 separates the two losses.
