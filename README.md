...
不是五次没做对，是**在这个寻址方式下不可能**：上下文条件化取回需要"按查询取回不同内容"，
而取回键只有最近 2–3 个 token。（要绕开它，必须换成**查询相关寻址** —— 即真正的检索，
如 kNN-LM / RETRO 那一路，而不是 token n-gram 哈希。）详见 `docs/round-157-why-the-graft-is-a-prior-not-a-knowledge-channel.md`。
