...
#show: icml2024.with(
  ...
  abstract: [
    ...
    Two measurements fix the size of what remains. A probe of the frozen table recovers about 59% of the trigram top-1 that explicit counts reach on the same corpus, and more training makes it worse, so the loss belongs to the read-out rather than to undertraining. The obvious repair does not pay either: at matched storage a 4-gram address ties or beats 8-token, 16-token and longest-suffix memories at every budget and on every corpus.
...
+ The unsaturated regime that reveals the corpus's surface prior is measured on four arms over 600 items on one backbone, without the reader-disabled arm that would make its noise floor computable; it is a secondary regime and is reported as one. The saturated null it qualifies is measured on two backbones.
...
Two measurements fix the size of the remaining opportunity, and both are discouraging for the obvious next moves. The read-out recovers about 59% of the trigram top-1 that explicit counts reach, and more training makes it worse, so the loss is architectural rather than a matter of budget. Lengthening the key does not pay at matched storage on any corpus or budget we tested. What is left is the complement: retrieval, which addresses by query, and parametric adapters, which are not a memory channel at all. The system results in this paper are consistent with that division and are re-read under it.
